...
# for rs/ via Cargo path dependencies on sibling checkouts of parser,
# json, jsonic and support.
...
# The doctests are run separately: `--all-targets` excludes them, and the
# README's examples are doctests of the crate.
test-rs:
...
# tabnas-json5 is not published. Only the constants need to stay in step.
version-rs:
