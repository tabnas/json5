...
// `expected` column also accepts the bare tokens NaN / Infinity / -Infinity
// and UNDEFINED. Returns [matched, value].
function specialExpected(raw: string): [boolean, any] {
  switch (raw) {
    case 'NaN': return [true, NaN]
  ...
  describe('spec: ' + file, () => {
    ...
    for (const row of rows) {
      test(`row ${row.line}: ${label(row.input)}`, () => {
        ...
        const tn = new Tabnas().use(jsonic).use(Json5, opts)
        ...
        const [special, wantSpecial] = specialExpected(row.expected)
        if (special) {
          if ('number' === typeof wantSpecial && Number.isNaN(wantSpecial)) {
            assert.ok(Number.isNaN(raw as any), `${file}:${row.line}: want NaN`)
          } else {
            assert.strictEqual(raw, wantSpecial, `${file}:${row.line}`)
          }
          return
        }
