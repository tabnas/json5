...
fn version_looks_like_a_semver() {
    let parts: Vec<&str> = tabnas_json5::VERSION.split('.').collect();
    assert_eq!(
        parts.len(),
        3,
        "VERSION is not x.y.z: {}",
        tabnas_json5::VERSION
    );
    for part in parts {
        assert!(
...
fn version_matches_package_json() {
    ...
    assert_eq!(
        ...
        tabnas_json5::VERSION,
        "ts/package.json disagrees with VERSION; both are rewritten by admin/publish.sh at \
         release, so if you bumped one by hand, bump the other"
    );
}
