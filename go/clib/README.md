# libtabnasjson5 — the json5 parser as a C ABI

<!-- tabnas-clib-template: v1 — stamped by admin tasks/adopt-clib.sh;
     edit the template and re-stamp, not this file. -->

The json5 format parser as a C shared library, so languages with no
tabnas port can parse and validate json5 input. This is one of the
...
| Function | Returns |
|---|---|
| `tabnas_version()` | `{"ok":true,"lib":"libtabnasjson5","format":"json5","template":"v1"}` |
...
// link against libtabnasjson5; every call returns a JSON []u8 to free with
// c.tabnas_free.
```
