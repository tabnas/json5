...
#
# The engine, the jsonic base grammar (which takes the JSON core by path
# in turn) and the fixture runner are PATH DEPENDENCIES on sibling
# checkouts (rs/Cargo.toml: `tabnas = { path = "../../parser/rs" }`,
...
# the JSON core, the jsonic base grammar and the fixture runner). Those
# entries legitimately move
