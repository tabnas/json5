...
- **Literal control characters inside strings.** `JSON5SourceCharacter`
  ...
  The Rust engine's string lexer honours `string.allow_control` (see
  `jsonic/rs/AGENTS.md`), so the Rust port would follow TS the moment the
  grammar turned it on; until Go can, all three reject.
...
1. Bump all **three** version sites together — `ts/package.json`, `VERSION`
   in `ts/src/json5.ts` and `const VERSION` in `go/json5.go`. Drift is
